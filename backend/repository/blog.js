...
const GET_BLOGS_OF_USER_QUERY = `SELECT ${TABLE_BLOG_ID}, ${TABLE_BLOG_PUBLICATION_DATE}, ${TABLE_BLOG_WRITER}, ${TABLE_USER_NAME}, ${TABLE_BLOG_TITLE}, ${TABLE_BLOG_COVER_IMAGE_URL}, ${TABLE_BLOG_CONTENT} FROM ${TABLE_BLOG} JOIN ${TABLE_USER} ON (${TABLE_BLOG_WRITER} = ${TABLE_USER_USERNAME}) WHERE ${TABLE_BLOG_WRITER} = :1 ORDER BY ${TABLE_BLOG_PUBLICATION_DATE} DESC OFFSET :2*${PAGE_SIZE} ROWS FETCH NEXT ${PAGE_SIZE} ROWS ONLY`
...
const getUserBlogs = async (username, offset) => {
    const connection = await getConnection()
    const {rows} = await connection.execute(GET_BLOGS_OF_USER_QUERY, [username, offset])
    connection.release()
    return rows
}

module.exports = {insertBlog, getAllBlogs, getUserBlogs}